...
int main(int argc, char** argv)
{
    ...
    fp = fopen("fichier.txt","w"); //on ouvre le fichier dans le quel on va garder nos traces d'executions.
    ...
    while (max_error > threshold)
    {
        ...
        for (i = 0; i < N; i++)
        {
            ...
            if (Nodes[i].con_size != 0)
            {
                if (iterations == 0)
                {
                       fprintf(fp,"%d:",i);// c'est pour mettre le point qui est manipulé
                ...
                for (j = 0; j < Nodes[i].con_size; j++)
                {
                    ...
                    if (iterations == 0)
                    {
                        if (j==0)// ici c'est si c'est le premier voisin qui  est manipulé
                        {
                            fprintf(fp,"%d",Nodes[i].To_id[j]);// ici c'est le voisin qui est manipulé
                        }

                        else // ici c'est un autre point 
                        {
                                  fprintf(fp,",%d",Nodes[i].To_id[j]);// ici c'est le voisin qui est manipulé
                        }
                    }
                ...
                fprintf(fp, "\n");// on ferme le fichier qui contient la trace d'execution
